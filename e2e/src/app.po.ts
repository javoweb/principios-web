...
export class AppPage {
  ...
  populateComments(comment: string, rating: number) {

    element(by.css("#albumDetails *[ng-reflect-name='rating']")).sendKeys(rating);
    element(by.css("#albumDetails *[ng-reflect-name='description']")).sendKeys(comment);
    element(by.css("#albumDetails *[ng-reflect-name='collectorId']")).sendKeys(Key.DOWN);

  }

  confirmSavedComment(comment: any): any {
    return element(by.xpath("//li[text()[contains(.,'"+comment+"')]]")).getText() as Promise<string>;
  }
  ...
  populateTrackFields(trackName: string, trackDuration: string) {
    ...
    element(by.css("#albumDetails input[name='duration']")).sendKeys(trackDuration);

  }


  getAlertFor(containerRef: string, fieldName: string , fieldType: string): Promise<string> {

    element(by.css(""+containerRef+" *["+fieldType+"='"+fieldName+"']")).sendKeys(Key.LEFT);
    browser.sleep(500);

    element(by.css(""+containerRef+" *["+fieldType+"='"+fieldName+"']")).click();
    browser.sleep(500);


    element(by.css(""+containerRef+" *["+fieldType+"='"+fieldName+"']")).sendKeys(Key.TAB);
    browser.sleep(500);
    element(by.css(""+containerRef+" *["+fieldType+"='"+fieldName+"']")).sendKeys(Key.TAB);
    browser.sleep(500);

    browser.sleep(500);
    return element(by.xpath("//*[@"+fieldType+"='"+fieldName+"']/../*[contains(@class, 'alert')] ")).getText() as Promise<string>;
